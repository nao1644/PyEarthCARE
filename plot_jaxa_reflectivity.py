#!/usr/bin/env python3
"""Plot EarthCARE CPR radar reflectivity from a JAXA CPR_ECO file."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "integrated radar reflectivity from a JAXA CPR_ECO file."
        )
    )
    parser.add_argument("input_file", type=Path, help="Input JAXA CPR_ECO HDF5 file.")
    ...
    parser.add_argument(
        "--resolution",
        choices=tuple(JAXA_REFLECTIVITY_VARIABLES),
        default="1km",
        help="JAXA integrated reflectivity product: 1km or 10km (default: 1km).",
    )
...
def make_plot(
    ...
    low_reflectivity_mask = np.where(
    ...
    fig.suptitle(
        f"EarthCARE, {JAXA_REFLECTIVITY_PRODUCT}: {variable_name}",
        fontsize=9,
        y=0.97,
    )
...
def main() -> None:
    ...
    latitude, height, reflectivity, variable_name = read_jaxa_reflectivity(
    ...
    print(f"Product:    {JAXA_REFLECTIVITY_PRODUCT}")
